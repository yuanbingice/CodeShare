...
//		titleScrollView.backgroundColor = _titleScrollViewColor ? _titleScrollViewColor:[UIColor colorWithWhite:1 alpha:0.7];
		...
		// 设置标题滚动视图尺寸
		titleScrollView.autoresizingMask = UIViewAutoresizingFlexibleWidth;
...
- (UIScrollView *)contentScrollView
{
    if (_contentScrollView == nil) {
		...
		_contentScrollView.autoresizingMask = UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
...
- (void)viewDidLayoutSubviews {
	...
	self.contentLayout.itemSize = CGSizeMake(self.view.width, self.view.height);
...
- (void)setUpTitleWidth
{
	...
	// 改了宽度
	CGFloat titleW = YZScreenW - self.titleMarginLeft * 2;
    ...
    if (totalWidth > titleW) {
		
		_titleMargin = margin;
		
		self.titleScrollView.frame = CGRectMake(self.titleMarginLeft, 0, self.view.width - self.titleMarginLeft * 2, titleH);
		
		self.titleScrollView.contentInset = UIEdgeInsetsMake(0, 0, 0, _titleMargin);
		
	}else {
		CGFloat titleMargin = (titleW - totalWidth) / (count + 1);
		
		_titleMargin = titleMargin < margin ? margin : titleMargin;
		
		self.titleScrollView.frame = CGRectMake(self.titleMarginLeft, 0, titleW, titleH);
	}
...
- (void)setUpAllTitle
{
    ...
    // 设置标题滚动视图的内容范围
    UILabel *lastLabel = self.titleLabels.lastObject;
	_titleScrollView.contentSize = CGSizeMake(CGRectGetMaxX(lastLabel.frame) + _titleMargin, _titleHeight);
